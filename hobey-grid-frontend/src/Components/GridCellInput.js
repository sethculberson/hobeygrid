...
export default function GridCellInput({
  ...
  return (
    <div className="relative">
      <input
        type="text"
        className={`w-full p-2 text-center text-white rounded-md focus:outline-none focus:ring-2 focus:ring-red-600 disabled:bg-gray-600 ${cellBgClass}`}
        value={value.name}
        ...
        <div className="absolute z-10 w-full bg-gray-700 border border-gray-600 rounded-md shadow-lg mt-1 max-h-48 overflow-y-auto">
          {isSearching ? (
            ...
            searchResults.map(player => (
              <div
                key={player.id}
                className="p-2 text-white hover:bg-red-800 cursor-pointer"
